...
import { useRef, Ref, useEffect, useContext, StateUpdater } from "preact/hooks";
import { createContext } from "preact";
import { ErrorMessage } from "./messages";
export const PWMatchContext = createContext<StateUpdater<boolean>>(() => true);

export const Username = ({ placeholder = "", isRequired = true }: { placeholder?: string, isRequired?: boolean }): JSX.Element => {
	...
	return <div>
		...
		<input type="text" placeholder={placeholder} name="username" id="username" ref={input} required={isRequired} autofocus minLength={2} pattern="^[A-Za-z0-9]+(?:[_][A-Za-z0-9]+)*$" /><br />
	</div>;
};
...
export const PWWithConfirm = ({
	...
	isRequired = true }: {
		...
		errorMessage: Ref<HTMLHeadingElement | null>,
		isNew?: boolean,
		placeholder?: boolean,
		isRequired?: boolean }): JSX.Element => {
